...
void OnMult(int m_ar, int m_br) 
{
   ...
   // Sol 2
  /* #pragma omp parallel for collapse(2) private(i, j, k, temp)
for(int i=0; i<m_ar; i++)
{
    for(int j=0; j<m_br; j++)
    {
    ...
    Time2 = omp_get_wtime();
    double executionTime = Time2 - Time1;

    long long FLOPs = static_cast<long long>(m_ar) * m_br * m_ar * 2;


    double GFLOPS = (FLOPs / executionTime) / 1e9;

    sprintf(st, "Time: %3.3f seconds\n", executionTime);
    cout << st;
    sprintf(st, "GFLOPS: %3.3f\n", GFLOPS);
    cout << st;
